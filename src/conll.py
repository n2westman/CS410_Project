...
labels_map = {'B-ORG': 'ORG', 'O': 'O', 'B-MISC': 'MISC', 'B-PER': 'PER',
              'I-PER': 'PER', 'B-LOC': 'LOC', 'I-ORG': 'ORG', 'I-MISC': 'MISC', 'I-LOC': 'LOC'}

caseLookup = {'numeric': 0, 'allLower': 1, 'allUpper': 2, 'initialUpper': 3, 'other': 4, 'mainly_numeric': 5,
            'contains_digit': 6}

mapping_files = {'en.train':'../../data/conll2003/eng.train.txt', 'en.testa':'../../data/conll2003/eng.testa.txt', 'en.testb':'../../data/conll2003/eng.testb.txt',
                 'de.train': '../../data/conll2003/deu.train.txt', 'de.testa': '../../data/conll2003/deu.testa.txt','de.testb': '../../data/conll2003/deu.testb.txt',
                 'es.train': '../../data/conll2002/esp.train.txt', 'es.testa': '../../data/conll2002/esp.testa.txt','es.testb': '../../data/conll2002/esp.testb.txt',
                 'nl.train': '../../data/conll2002/ned.train.txt', 'nl.testa': '../../data/conll2002/ned.testa.txt','nl.testb': '../../data/conll2002/ned.testb.txt',
                 'fifty_nine.cca.normalized':'../../data/fifty_nine.cca.normalized', 'cadec':'../../data/cadec/cadec.conll'}
...
class Conll_dataset():
    ...
    def __init__(self,opt, train=True, tag_type='ner'):
        ...
        if(opt.lang.lower()=='cadec'):
            inputs_word, inputs_char, inputs_case, labels = self.cadec(opt, tag_type=tag_type)
        else:
            inputs_word, inputs_char, inputs_case, labels = self.conll(opt, tag_type=tag_type)
        self.check_ids(self.train)
        self.check_ids(self.val)
        self.check_ids(self.test)
        # Build vocab
        inputs_char.build_vocab(self.train.inputs_char, self.val.inputs_char, self.test.inputs_char, max_size=opt.maxcharvocab)
        inputs_case.build_vocab(self.train.inputs_case, self.val.inputs_case, self.test.inputs_case)
        inputs_word.build_vocab(self.train.inputs_word, self.val.inputs_word, self.test.inputs_word, max_size=opt.maxvocab,
                                #vectors ="fasttext.en.300d")
                                vectors=[lib.data.Crosslingual('../../data/fifty_nine.cca.normalized')] if opt.pre_embs else None)
        ...
        if(opt.labeled != -1):
            ...
            ratio = opt.labeled/len(self.train)
            if ratio!=0:
                self.train, self.train_unlabeled = self.train.split(ratio)
            else:
                self.train_unlabeled = data.Dataset(examples=self.train.examples, fields=self.fields)
                self.train.examples = []
        if(opt.budget is None): opt.budget = len(self.train_unlabeled)
    ...
    def conll(self,opt, tag_type='ner'):
        ...
        """
        logger.info('---------- CONLL 2003 %s lang = %s ---------' % (tag_type, opt.lang) )
        ...
        inputs_word = data.Field(batch_first=True, fix_length=opt.maxlen, lower=opt.lower,
                                 preprocessing=data.Pipeline(lambda w: '0' if opt.convert_digits and w.isdigit() else w))

        inputs_char_nesting = data.Field(tokenize=list, batch_first=True, fix_length=opt.maxlen)
        inputs_char = data.NestedField(inputs_char_nesting)

        inputs_case = data.Field(batch_first=True, fix_length=opt.maxlen,
        ...
        if(opt.lang == "en"):
            self.fields = ([(('inputs_word', 'inputs_char', 'inputs_case'), (inputs_word, inputs_char, inputs_case))] +
                  [('labels', labels) if label == tag_type else (None, None) for label in ['pos', 'chunk', 'ner']] +
                  [('id', id)])

        elif(opt.lang == "de"):
            self.fields =([(('inputs_word', 'inputs_char', 'inputs_case'), (inputs_word, inputs_char, inputs_case))] + [('idk', None)] +
                          [('labels', labels) if label == tag_type else (None, None) for label in ['pos', 'chunk', 'ner']] +
                          [('id', id)])

        elif(opt.lang == "nl"):
            self.fields =([(('inputs_word', 'inputs_char', 'inputs_case'), (inputs_word, inputs_char, inputs_case))] +
                          [('labels', labels) if label == tag_type else  (None, None) for label in ['pos', 'ner']] +
                          [('id', id)])
        else:
            self.fields =([(('inputs_word', 'inputs_char', 'inputs_case'), (inputs_word, inputs_char, inputs_case))] +
                      [('labels', labels) if label == tag_type else (None, None) for label in ['ner']] +
                      [('id', id)])
    ...
    def cadec(self,opt, tag_type='ner'):
        ...
        self.fields = ([(('inputs_word', 'inputs_char', 'inputs_case'), (inputs_word, inputs_char, inputs_case))] +
                       [('labels', labels) if label == tag_type else (None, None) for label in ['ner']] + [('id', id)])
        ...
        self.train, self.val, self.test = datafile.split(split_ratio=[5610, 1000, 1000])
        return inputs_word, inputs_char, inputs_case, labels
    ...
    def keep_duplicates(self):
        self.temp_train = data.Dataset(examples=self.train.examples, fields=self.fields)
        self.temp_val = data.Dataset(examples=self.val.examples, fields=self.fields)
        self.temp_test = data.Dataset(examples=self.test.examples, fields=self.fields)
    ...
    def batch_iter(self, batch_size):
        if(self.opt.adaptive_batch_size): batch_size = int(math.ceil(len(self.train)/self.opt.adaptive_batch_size))
        # Get iterators
    ...
    def sample_unlabeled(self, k_num):
        ...
        sampled_examples = [example for count, example in enumerate(unlabeled_pool.examples) if count in indices[:k_num]]
    ...
    def sample_validation(self, k_num):
        ...
        np.random.shuffle(indices)
        sampled_examples = [example for count, example in enumerate(validation_pool.examples) if count in indices[:k_num]]
    ...
    def get_prediction(self, example, fields, model): #Predict label
        ...
        pseudo_example = data.BucketIterator(dataset=pseudo_example, batch_size=1, repeat=False, shuffle=False,
                                             device=torch.device("cuda:"+str(self.gpu) if self.gpu!=-1 else "cpu"))
        ...
        if isinstance(model, torch.nn.Module):
            ...
            y = list(filter(lambda x: x!= pad, pseudo_example.labels.data.tolist()[0]))
        else:
        ...
        prediction = lib.utils.indices2words(prediction, model.wordrepr.tag_vocab)
        ...
        prediction = prediction[:len(y)] #shrink prediction to same len as labels
        return prediction